...
static int hgemmBatch(cb_order order, cb_transpose transA, cb_transpose transB,
  ...
  for (i = 0; i < batchCount; i++) {
    ...
    err = CLBlastHgemm(convO(order), convT(transA), convT(transB), M, N, K,
                      (cl_half)alpha, A[i]->buf, offA[i], lda, B[i]->buf, offB[i], ldb,
                      (cl_half)beta, C[i]->buf, offB[i], ldc, &ctx->q, &ev);
    if (err != kSuccess)
...
static int hgemv(cb_order order, cb_transpose transA, size_t M, size_t N,
  ...
  err = CLBlastHgemv(convO(order), convT(transA), M, N, (cl_half)alpha,
                    A->buf, offA, lda, X->buf, offX, incX,
                    (cl_half)beta, Y->buf, offY, incY, &ctx->q, &ev);
  if (err != kSuccess)
    return GA_BLAS_ERROR;
...
static int hgemm(cb_order order, cb_transpose transA, cb_transpose transB,
  ...
  err = CLBlastHgemm(convO(order), convT(transA), convT(transB), M, N, K,
                    (cl_half)alpha, A->buf, offA, lda, B->buf, offB, ldb,
                    (cl_half)beta, C->buf, offC, ldc, &ctx->q, &ev);
  if (err != kSuccess)
    return GA_BLAS_ERROR;
...
static int hger(cb_order order, size_t M, size_t N, float alpha,
  ...
  err = CLBlastHger(convO(order), M, N, (cl_half)alpha, X->buf, offX, incX,
